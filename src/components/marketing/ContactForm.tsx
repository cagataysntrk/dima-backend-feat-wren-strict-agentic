...
export function ContactForm({ locale }: { locale: MarketingLocale }) {
  ...
  const t = text[locale];
  if (state.status === "success") return <div role="status" className="rounded-lg border border-chart-2/40 bg-chart-2/10 p-6 text-sm leading-6">{state.message}</div>;
  const fields = [
    ["fullName", t.name, "text"], ["email", t.email, "email"], ["company", t.company, "text"], ["role", t.role, "text"], ["dataSource", t.source, "text"],
  ] as const;
  return (
    <form action={action} className="space-y-5" noValidate>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        {fields.map(([name, label, type], index) => (
          <div className={index === 4 ? "sm:col-span-2" : ""} key={name}>
            ...
            <Input className="mt-2 min-h-11" id={name} name={name} type={type} required={name !== "dataSource"} aria-invalid={!!state.errors?.[name]} aria-describedby={state.errors?.[name] ? `${name}-error` : undefined} />
      ...
      <p className="text-xs leading-5 text-muted-foreground">{t.privacy} <Link className="underline hover:text-foreground" href="/privacy">{t.link}</Link></p>
