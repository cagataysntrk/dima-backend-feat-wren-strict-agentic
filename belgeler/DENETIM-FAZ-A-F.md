...
| **C2** | bütçe + stall | ✅ *(kapı 8)* | ✅ **KAPANDI (08-12)** — ◐ **ürün ✅ / kapı eksik**: kapı **7** test (8 değil) · kapı **yalnız varsayılanı** ölçüyor · 🔴 **İKİ CANLI ÇAĞRI `sorgu=0` geçiyor** (`routers/ask.py:273`, `answer.py:567`) = *o eksende sınırsız* — kapının **adıyla yasakladığı hâl**, ve gerekçesi yazılı değil | ✅ |
...
| **C3③** | dört kapı | ✅ | ✅ **KAPANDI (08-12)** — 🔴 **KAPI SAHTE-YEŞİLDİ** — `assert "Planlayici" in inspect.getsource(...)`; iki jeton da `cagir`'ın **docstring'inde** geçiyor → gövdedeki çağrı silinse kapı yeşil kalır (`test_c3…:152-155`) | ✅ |
| **C3④** | sanitizasyon | ✅ | ✅ **KAPANDI (08-12)** — 🔴🔴 **GÜVENLİK — ENGELLENMİYORDU.** `mcp.py:181` tek geçişli `str.replace`, ve yerine konan dize sentinel'in **öneki/soneki**: `ZARF_SON` → `">>>"`. Sonuç: `"DIMA-VERI"+"DIMA-VERI>>>"` → `"DIMA-VERI>>>"` **yeniden kuruluyor**. **KENDİ ÖLÇÜMÜM:** `_zarfla` çıktısında `ZARF_SON` **2 kez** → zarf **erken kapanıyor**, hücredeki *«YENİ TALİMAT»* beyan edilmiş veri bölgesinin **dışında** kalıyor. Kapının yüklemi doğru, **test vektörü** eksik (`test_c3…:108` yalnız tek sentinel besliyor) | ✅ |
| **C4/C5** | — | — | ✅ **YOK** — `§14.1:3197-3199` FAZ 2'yi `C1·C2·C3` olarak sayıyor; kapsam tam | ⊘ |
| **+** | `§14.11 D9` MCP borcu | işaretsiz | ✅ **KAPANDI (08-12)** — kart İKİYE bölündü: ⊘ `400+agent_error` reddedildi (3 gerekçe) · ✅ `_meta.belirsizlik` alındı. Önce: 🔴 **ŞART GERÇEKLEŞTİ, İŞ YAPILMADI**: `agent_error`/`ambiguous_measure` `app/` altında **0**. Canlı: `/ask` → `suggestions[{kind:"tanim",…}]` **yapısal**; `/mcp/call route` → `text:"null"`. Aday üreten üç fonksiyon `tools.KAYIT` **dışında** — bilgi *kaybolmuyor*, **hiç üretilmiyor**. Ajan `null` alıyor: bu *«dürüst red»* bile değil, **sessiz red** | ✅ |
| **+** | `§14.16 E` Wren ad hizalaması | — | ✅ **KAPANDI (08-12)** — ⊘ ad hizalaması reddedildi (4 gerekçe) ama ölçüm daha ağırını buldu: **5 araç çağrılamıyordu**; `enjekte` gerçekten kuruldu + `katalog` aracı. Önce: 🔴 **İKİNCİ SAYILMAMIŞ BORÇ**: `query_cube`·`list_cubes`·`describe_cube`·`get_context`·`recall_queries` **hiçbiri** yok; şartı (`C3` açılışı) doldu, karar **kararsız** | ✅ |
| **+** | `tools.get` hata mesajı | — | ✅ **KAPANDI (08-12, üç yerde)** — ⚠ `KeyError` **tüm 31 aracı** listeliyor (`tools.py:839`), `principal`'a göre **süzülmüyor** → rol matrisi açıldığı gün **envanter sızıntısı** | ✅ |
| **+** | MCP bütçesi | — | ✅ **KAPANDI (⏸ oturum bütçesi ayrı karar; `_meta.kalan` eklendi)** — ⚠ `routers/mcp.py:59` her istekte **taze** `Planlayici(butce=Butce())` → `§C2` tavanı **oturum değil çağrı** tavanı; ajan N çağrıyla N×tavan harcayabilir | ✅ |
...
| # | kalem | rapor | **ÖLÇÜLEN** | durum |
|---|---|---|---|---|
| **E1** | Adtributor | ◐ | ⊘ dürüst — ama gerekçe zinciri kırık: *«E3 ön koşuldu, yapıldı»* deniyor; E3 kartın saydığı **4 tarama yerinin 1'ini** kapsıyor | ✅ *(⟳ zincir onarıldı 08-12: E3 dört tarama yerinin **dördünü** de kapsıyor; ön koşul artık GERÇEKTEN sağlanıyor — `test_e3_secim_genisligi.py` kapsamı sayıyla kilitliyor)* |
...
| **§38 D13** dış yüzey (MCP) | ✅ | 🔴🔴 **İŞARET FAZLA İYİMSER** — aşağıdaki `D9/MCP` bölümüne bak: mekanizma ✅ ama **yayımlanmış sözleşme YANLIŞ** | ✅ *(yayım sunulabilene türetildi (32→29), kapı 12)* |

## D-B · `§14.1` FAZ 3 YOL HARİTASI (D1–D5)

| kimlik | rapor | **ÖLÇÜLEN** | durum |
|---|---|---|---|
| **§14.1 D1** olgu sayacı | ✅ başlık / ⏸ kart | ✅ **kapı doğru şeyi ölçüyor** (bağımsız `ast` ölçümü regex'le birebir → kör nokta yok). ⚠ **aynı satırda iki işaret** (`:3732` ✅ ↔ `:3777` ⏸) | ✅ *(işaret `✅◐` (ölçüm ✅ / tesisat ⏸))* |
| **§14.1 D2** taksonomiyi aç | **işaretsiz** | 🟡 **KARARI YAZILMAMIŞ** — D1'in ölçümü kalemi *«gereksiz»* ilan etmiş, `_streak` eşiği 3 aklanmış, ama kalem ne ✅ ne ⊘ (`:3787`). `§40`'ın kendi kuralına aykırı | ✅ *(⊘ önülü karşılandı — gerekçe doğru dosyaya bağlandı)* |
...
| **§14.1 D4** ön-uç sayı biçimi | ✅ | ✅ kapı 5/5 — 🔴 **RAPOR BAYAT**: *«`maliyet.ort_kar_marji_yuzde` kapıda `OLCULMEDI` olarak kayıtlı»*; ölçüldü **`OLCULMEDI = set()` BOŞ**, kalem canlı ölçülüp (28,13) kanıtlı `OLCULDU_ZATEN_YUZDE`'ye taşınmış. **Kod rapordan ileride** | ✅ *(bayat cümle tazelendi (`OLCULMEDI` bugün BOŞ))* |
| **§14.1 D5** takip anlama | ◐ *(5)* | ✅ **dürüst ◐** — kapı **6** test; kart *«odak varlığı yok»* diyor, gövde onu çürütüyor ama işaret ◐ doğru | ✅ *(kapı sayısı 6'ya tazelendi)* |
