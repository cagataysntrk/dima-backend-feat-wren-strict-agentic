...
| ✅ **D2** | Garsona örnek verme | ❌ **yok** — istem yalnız katalog + soru | ⟳ **ÖLÇÜLDÜ (08-12): `vqr.ara` HİÇ YAZILMADI** (`grep` → **0**) ve **gerekmedi** — iş var olan `vqr.few_shot_block` ile yapıldı (`app/vqr.py:449` → `app/routers/ask.py:4093-4096`, bayrak `beta`). Bu satır `:3573`'te **zaten çürütülmüştü**, ama SONRA/dosya sütunları **bayat kalmıştı**. İşaret ✅ doğru, gerekçe tazelendi. ⊙ *Bir kartın yapılacağı, başka bir adla çoktan yapılmış olabilir.* | `app/vqr.py::few_shot_block` · `app/llm.py::_cube_select_system` · `app/routers/ask.py` | **§4 LLM rolleri** — *«garson few-shot alır»* |
...
| ✅ **D7** | Yetki denetimi (plan) | ⟳ **ÖLÇÜLDÜ (2026-08-12): BOŞLUK YOK.** Plan fiilleri gerçekten `Planlayici.calistir()`'den geçmiyor (`grep authorize` = 0) — **ama arkasında bir şey yok**: `SORGU` isteğin **zaten yetkilendirilmiş** servisini kullanır, ötekiler **koşmuş satırlar** üstünde saf dönüşümdür, `PANO` ise **YAZMAZ** (`pano_taslagi` döndürür; kalıcılaştırma onay akışının işi). *Bir kapının yokluğu ancak arkasında bir şey varsa boşluktur.* ⟳ **BU TEŞHİS `§14.14 F13` İLE REVİZE EDİLDİ (08-12).** *«Arkasında bir şey yok»* cümlesi o gün doğruydu ama **kalıcı değildi**: `F13` yazma araçlarını kademelendirince `calistir()`'e **beşinci bir kapı** eklendi — `Planlayici._onay_kapisi` (`app/planner.py:348`, çağrı `:549`), ölçüldü. İşaret ✅ kalıyor (salt-okunurluk değişmezi hâlâ geçerli ve kapılı) ama **gerekçe** artık *«arkasında bir şey yok»* değil, *«arkasındaki tek şey ONAY kapısıdır ve o da kapılıdır»*. ⊙ *İki kart aynı fonksiyonu adıyla anıp zıt sonuca varıyorsa, ikisi de ✅ olamaz — biri ötekinin ölçümünü henüz okumamıştır.* | 🔴 **GERÇEK DEĞİŞMEZ: çalıştırıcı SALT-OKUNUR ve idempotent** — ve o cümle yalnız bir **yorumda** duruyordu. Artık kapılı: `tests/test_d7_plan_salt_okunur.py` (**6** ⟳08-12; 5 bayattı) — router ithali yok · yazma fonksiyonu adı geçmiyor · `PANO` taslak döndürüyor · `sorgu_kos` yalnız `SORGU` dalında (SAYARAK) | `app/plan_kosucu.py` · `tests/test_d7_plan_salt_okunur.py` | **§ güvenlik** |
| ✅ **D8** | Durdurma koşulu | ⟳ **ÖLÇÜLDÜ (2026-08-12): İKİ YARISI DA YAPILMIŞ — ve bu satır `§C2` ile AYNI İŞ.** Bu tablo aynı işi iki kez listeliyor (`C2` kartı *«ZATEN CANLI»* diye kapatmıştı); `D8`'in *«ÖNCE»* metni o ölçümden **önceki** hâli taşıyordu. Bugünkü ölçüm: `Butce(adim=**8** · saniye=**30,0** · sorgu=**12**)` — `sorgu` **uykuda değil**, `if b.sorgu and …` uygulanıyor ve tavan aşılınca `ButceAsimi` **fırlıyor**; `AZAMI_ADIM=12` `dogrula`'da **koşmadan önce**. | ✅ **Stall yarısı da tam:** `ONARIM_TAVANI = **2**` (Magentic-One *«stall ≤2»*) ve döngü **yeniden PLANLIYOR** — istem sınıf başına **çare** taşıyor (`ONARIM_YONERGESI`, `§B4`), yalnız *«tekrar dene»* demiyor; tavan dolunca **durur**. ⊙ İki kayıt tek işi anlatınca biri bayatlar: *bir görevin iki satırı varsa, ikincisi birincinin ölçümünü duymaz.* ⟳ **ÇİFT SAYIM KAYDA GEÇTİ (08-12):** bu satır `§14.1 C2` ile aynı işi anlatıyor, toplamlarda **bir kez** sayılır (`§14.15`). ⊙ Kapı sayısı **(8) DOĞRU** — denetim ajanı *«gerçekte 7»* demişti; `--collect-only` ile ölçtüm: **8**. | `app/planner.py` · `app/plan_garson.py` · kapı `tests/test_c2_butce_stall.py` (8) | **§2.0** |
| ◐ **D9** | Metodoloji (kohort/funnel/YoY-oranı) | ❌ yok — plan fiilleri + şablon | ✅ **ÜÇÜ DE YAZILDI (2026-08-12)** — mekanizma `§B10`'da kurulmuştu, eksik olan **metinlerdi**. 🔴 Ve ikisi **zıt**: `huni.md` *«YAPILABİLİR»* der (ölçüldü: `firsat` küpü **zaten bir satış hunisi** — sinonimleri `huni`·`pipeline`, `asama` boyutu, gerçek aşama değerleri, `kazanma_orani_yuzde` ölçüsü); `kohort.md` *«İFADE EDİLEMEZ»* der (ölçüldü: kohort **ilk dönem** + **görece zaman** ister, küp sözleşmesinde ikisi de yok — veri var, **ifade gücü** yok). ⊙ `kohort.md`'nin tamamı bir *«yapamam ama şunu BEYAN et»* talimatıdır ve `§18.6`'da ölçülen kusuru hedefler (pivot, kohort diye teslim ediliyordu). 🔴 `huni.md`'nin en önemli satırı bir **sınır**: `asama` bugünkü aşamayı taşır, **dönüşüm oranı aşama geçmişi ister ve o yok** — anlık dağılımı dönüşüm diye sunmak sessiz-yanlıştır. ⏸ Bayrak `skills: off` **kalıyor** (açılış şartı `§26`, park); `KURAL B` kapıyla ölçülüyor. ⟳🔴 **İŞARET DÜZELTİLDİ ✅ → ◐ (08-12).** Üç metin **yazıldı** ve yükleyici (`katalog_metni.skills_metni`) onları **okuyor**, ama `demo/packs/features.yml:37` `skills: "off"` olduğu için metin garsonun kataloğuna **hiç eklenmiyor** — iş **yazıldı, bağlanmadı**. Aynı işi `§14.14 B10` *«KISMEN KAPANDI»* diye işaretliyordu; iki satır artık **aynı işareti** taşıyor. ⊙ *Düz bir ✅, bayrağın arkasındaki bir metni çalışıyor gibi okutur* (`§12.12`). | `demo/skills/{yoy-orani,huni,kohort}.md` · kapı `tests/test_d9_metodoloji_skilleri.py` (11) | **yeni ADR** |
...
| ✅ **D13** | Dış yüzey | `mcp_yuzeyi: off` · `agent_plan_secimi: off` | ✅ **MCP AÇILDI (`beta`, 2026-08-12)** — ve açılış bir dilek değil, **dört ölçülmüş şartın** karşılanmasıydı (`§C3`). 🔴 Son engel `§C3`'ün kendi cümlesiydi: *«`§C1` (tek yetenek kaydı) hâlâ `mcp_yuzeyi` açılışına bağlı»* — o borç **bu turda `§D6` ile ödendi** (15/15 fiil–araç eşleşmesi, içe aktarmada doğrulanıyor). ⚠ *«~20 araç eşiği»* korunmadı, **yerine geçildi**: OpenAI'ın asıl ölçütü sayı değil **ÖRTÜŞME**dir — ölçüldü, 31 aracın **29'u ayrık**, tek örtüşen çift `stats.trend`↔`stats.ozet` (tavan 10); sayı ikincil emniyet olarak 31 ≤ 35. *Bir vekil ölçüt kırmızı verirken asıl ölçüt yeşilse, değiştirilecek olan vekildir.* | 🟢 **CANLI DOĞRULANDI (curl, üç yön):** `GET /mcp/tools` → **200 · 31 araç · yazma aracı SIFIR** · `POST /mcp/call` → çalışıyor ve hem **enjeksiyon zarfını** (`<<<DIMA-VERI … DIMA-VERI>>>`) hem **makbuzu** (`arac`·`determinizm`·`sure_ms`) taşıyor · `dashboards.create` → **reddedildi** (*«Kayıtlı olmayan araç»*). ⏸ `agent_plan_secimi` **açılmadı** — ayrı bir ürün kararı. | `demo/packs/features.yml` · `app/mcp.py` · kapılar `test_c3_mcp_acilis_sartlari.py` · `test_mcp.py` | **§ MCP** | ⟳🔴 **08-12: «CANLI DOĞRULANDI» FAZLA İYİMSERDİ.** Mekanizma ✅ ama **yayımlanan sözleşme YANLIŞTI**: `mcp.araclar()` 31 aracın **77 alanının tamamını `"string"`** ilan ediyordu, oysa gerçek imzalarda `object` **31** · `array` **14** · `boolean` **7** · sayısal **5** var. Canlı ikili kanıt: `stats.ozet` **şemaya uyan** çağrıda `isError`, **şemayı yok sayan** çağrıda `200`. Merdivenin birinci basamağı `route` bile çağrılamıyordu. ✅ **ONARILDI:** tip artık `modul.fonksiyon` → `inspect.signature` ile **gerçek imzadan** türüyor; yayım `{object 24 · array 11 · integer 2 · boolean 1 · string 39}` — **38 alan** düzeldi, `route` artık `{question: string, schema: object}`. Kapı `test_c3_mcp_sozlesmesi_gercek.py` (5). ⊙ *Tek bir başarılı çağrı, 31 araçlık bir sözleşme için kanıt değildir.* ⟳🔴 **VE İKİNCİ BİR YALAN DAHA ÖLÇÜLDÜ (08-12, denetim ⑬⑥):** `llm.prompt_enhance` · `llm.anlat` · `llm.select_cube` `tools/list`'te **görünüyordu** ama `baglanma="servis:llm"` ve `routers/mcp.py` yalnız `{"servis:wren"}` sağlıyor → çağrı `ValueError` ile düşüyordu. Yani **üç araç ilan edilmiş ama sunulamıyordu**; dış çağıran onları deneyip hatayı **kendi isteğinde** arardı. ✅ **ONARILDI:** yayımlanan liste artık ucun **gerçekten sağladığı** kaynak kümesinden (`_KAYNAK_ADLARI`) **türetiliyor** — bir dışlama listesi değil (`KAT-1`); yarın `servis:llm` verilirse üçü **kendiliğinden** görünür. Ölçüldü: **32 → 29**. ⊘ `servis:llm` **bilerek yok**: dış bir çağıranın LLM bütçesi harcaması ayrı bir **yönetişim** kararıdır ve ilan edilmeden verilemez. ⊙ **Ve ajanın öteki iki iddiası ÖLÇÜLDÜ, ikisi de KAPANMIŞ:** *«23 araç dict/list bekliyor ama string ilan ediliyor»* → tip uyuşmazlığı **0**; *«5 zorunlu parametre eksik»* → şemada olmayan ama `enjekte` de olmayan parametre **0** (13'ünün hepsi beyanlı enjeksiyon). Kapı `test_c3_mcp_sozlesmesi_gercek.py` (**12**), mutasyonlu. *Bir aracı yayımlamak onu çağrılabilir yapmaz.*
...
> ⟳ **İŞARET AYRIŞTIRILDI (08-12):** başlık **✅**, gövde **⏸ PARK** diyordu ve
> ikisi de doğruydu — ama tek bir işaret **iki farklı şeyi** anlatıyordu.
> Doğrusu **`✅◐`**: **ölçüm** tarafı kapandı (sayaç ölçüldü, teşhis düzeltildi),
> **tesisat** tarafı `§38.3 D11` ile birlikte **park**ta. *Bir kartta iki işaret
> varsa, okuyan hangisinin bugünü anlattığını bilemez.*
...
> sandı (**yanlış pozitif**); değeri doğrulanamadığı için **dokunulmadı**.
> ⟳ **BU CÜMLE ARTIK BAYAT (08-12):** kalem o günden sonra **canlıda ölçüldü**
> (**28,13** → zaten 0–100) ve kapıda `OLCULMEDI`'den `OLCULDU_ZATEN_YUZDE`'ye
> **kanıtıyla** taşındı; bugün `OLCULMEDI` kümesi **boş** (`tests/
> test_d4_yuzde_olcegi.py:62`). İşaret ✅ doğru — *ama bir kartın «kapıda şöyle
> kayıtlı» cümlesi, kapının bugünkü hâliyle sınanmadıkça bir hatıradır.* *Bir ölçek düzeltmesi, ölçeği ölçülmemiş bir
...
> Kapı: `tests/test_at_atif_beyani.py` (**6** ⟳08-12; 5 bayattı) + takip yüzeyi **269 yeşil**.
>
...
| **risk** | ⚠ Ölçüm gürültüsü kök neden diye sunulabilir → **E3 ön koşul** ⟳✅ **ZİNCİR ONARILDI (2026-08-12).** Bu satır *«E3 ön koşuldu, yapıldı»* diye okunuyordu; denetim ölçtü — `stats.tarama_beyani`'nin **tek** çağrı yeri vardı ve kartın saydığı **dört** tarama yerinin **biri** kapsanıyordu, yani ön koşul **sağlanmamıştı**. Bugün `stats.secim_beyani` eklendi ve **dördü de** bağlandı (`kok_neden._en_ayristiran` · `derinles` · `toplam_turu` · `contribution.arastir`; kapı `test_e3_secim_genisligi.py`, kapsam **sayıyla** kilitli). Ön koşul artık **gerçekten** sağlanıyor. ⊙ *Bir ön koşulu «yapıldı» diye işaretlemek, onun kapsamını ölçmeden yapıldığında bir sonraki kartın gerekçesini çürütür.* |
...
| # | kalem | kaynak | ne yapar | maliyet |
|---|---|---|---|---|
| ⏸ **E5** | **`ruptures`** (BSD-2) + **`statsforecast`** (Apache-2.0) | §13.8 | 🔴 **~~E2'nin (JS sürprizi) ÖN KOŞULU~~ ⟳🔴 **DEĞİLMİŞ — ÖLÇÜLDÜ (`§E2`, 08-11) ve bu satır o gün ÇÜRÜDÜ ama düzeltilmedi (08-12).** Kartın öncülü: Adtributor `F` (baseline/forecast) ister — *«beklenen değer»* olmadan *«sürpriz»* hesaplanamaz. Ayrıca *«mart'ta düştü»* iddiasını **doğrular** | günler ⟳ **KARAR:** **PARK (`§40.5`)** — `E1` Adtributor'ın ön koşulu; `§25` layer-1 kilidi açılmadan gereksiz. ⟳ **VE GEREKÇE SADELEŞTİ (08-12):** `§E2` forecast'sız kapandı — bizim `F`'imiz **önceki dönemin kendisidir** ve `yoy.compute` onu `*_gecen` kolonunda **zaten** veriyor. Yani `E5` `E2`'nin ön koşulu **değildi**; park kararı geçerli ama **başka** bir gerekçeyle (`§25` layer-1). ⊙ *Bir kartın iki yerde iki farklı gerekçesi varsa, biri ölçümü duymamıştır.* |
...
| ✅ **D11** | **Denormalizasyon** | §27.6 | ⟳ **ÖLÇÜLDÜ ve KAPANDI (2026-08-12).** Kartın hipotezi (*«JOIN'i cube derleyicisi kurar, risk yapısal düşük»*) **doğrulandı** ve artık **sayısı var**: **23 küp** · tablo dağılımı **{1 tablo: 16 · 2 tablo: 6 · 3 tablo: 1}** — 3+ tabloya çıkan **yalnız `kalite`** (`tamir_rework`+`makineler`+`partiler`). İlişki-türevi köken **9**; `hops` **9/9 → 1** *(hiçbiri 1'i geçmiyor)*; `certified` **9/9 → `olculdu:saglikli`** *(sertifikasız köken **yok**)*. Literatürdeki `%20` **çok atlamalı** zincirlerde ölçülmüştür; bizim maruziyetimiz **tek atlama**. 🔴 **Kapı** `tests/test_d11_denormalizasyon_siniri.py` (5): *«5+ tablo **ya da** `hops>1` **ya da** `certified != olculdu:*` → KIRMIZI»* — bugün yeşil, üç eşiğin üçü de **mutasyonla** yük taşıdığı kanıtlandı. ⚠ **Denetim ajanının sayısı düzeltildi**: *«3+ tablo 7/23, `kalite` 4 tablo»* demişti — küp kimlikleri doğru ama sayılar **tam bir tablo kaymış** (taban `base_object`'tir ve `relationship` bir **kenardır**, tablo değil); ajanın *«7»*'si aslında **2+ tabloya** çıkan küp sayısıdır. |
...
| ⊘ **F14** | **`SessionContext`'in dört kullanılmayan yeteneği**: `get_available_functions` (motorun desteklediği fonksiyonları **sormak**, varsaymak yerine) · `transform_sql` · `pushdown_limit` · `list_tables` | FAZ 5 | günler ⟳ **KARAR: AÇILMIYOR — ama GEREKÇE YENİDEN YAZILDI (⟳ 2026-08-12, `§F14` denetimi).** 🔴 **Eski gerekçenin iki dayanağı da ölçümle çürüdü.** ⓐ *«`dry_run` ZATEN kullanılıyor»* → `ast` ile gerçek `Call` düğümü sayıldı: `dry_run` **app 0 · lab 0 · tests 0**. Depodaki dört isabetin **hiçbiri** motor çağrısı değil (`test_tenant_connections.py:26` ve `test_db_introspect.py:61,65` ayrı sembol; `test_motor_rls_onkosul.py:141` bir **imza listesi içinde dize**). ⓑ *«bu yüzeyde yok (fabrika arkasında)»* → **iki yüzey karışmış**: `WrenEngine` **4 metot** (`close·dry_plan·dry_run·query`) ↔ `wren_core.SessionContext` **10 metot** — dördü de **orada VAR**, ve depo kendi testlerinde `SessionContext`'i **doğrudan kuruyor** (`transform_sql` **7** çağrı: `test_motor_rls_onkosul.py:77,87,98,108` · `test_motor_rls.py:187,204` · `test_motor_cls.py:102`). Yani erişilemez değil, **kullanılmıyor**. ⊙ Çelişki depoda **zaten kayıtlıydı**: `tests/test_envanter_iddiasi_taze.py` bu satırı *«`dry_run` ✅ kullanılıyor» ↔ app'te **0** çağrı*» diye listeliyor — kapı görmüş, kart güncellenmemişti. **ÖLÇÜLMÜŞ YÜZEY (08-12, kendi koşumum):** `dry_plan` app **25** · `query` app **33** · `close` app **6** · `cube_query_to_sql` app **2** · `dry_run` **0** · `transform_sql` app **0**/tests **7** · `pushdown_limit` **0** · `list_tables` motor kullanımı **0** *(3 isabet başka sembol: `MssqlSource.list_tables`, `admin_app/clone`)* · `get_available_functions` **0**. ✅ **KARAR DEĞİŞMİYOR ve şimdi DOĞRU dayanağı var: `dry_plan` = 25 çağrı** — her sorgu koşulmadan önce motora doğrulatılıyor. ⏸ Şart aynı: `dry_plan`'ın kaçırdığı bir uyumsuzluk **canlıda ölçülene** kadar. ⚠ Üçü (`transform_sql`·`pushdown_limit`·`list_tables`) *«yüzeyde yok»* diye değil, **«ölçülmüş bir karşılığı yok»** diye ⊘ kalıyor. 🔴 **`get_available_functions` ayrı bir kaleme çıkarıldı** — bugün desteklenen fonksiyon kümesi bir **varsayımdır** ve `§B12` aynı sınıftan bir varsayımın ölçümle çürüdüğünü gösterdi (*«engel motorda değil VERİDE»*); motora **sormak**, *LLM önerir · motor doğrular* zincirinin eksik halkasıdır. ⊙ *Yanlış gerekçeli bir ⊘, bir sonraki turda «zaten ölçmüştük» diye yeniden okunmaz.* |
...
| **`wren serve mcp`** (`query_cube`·`list_cubes`·`describe_cube`·`get_context`·**`recall_queries`**) | §11.5 | ✅ **C3** — ⚠ *`recall_queries` bizim VQR'ımızın MCP karşılığı; C3'te araç adları hizalanmalı* | ⟳ **08-12 KARAR: AD HİZALAMASI ⊘ REDDEDİLDİ — ama ölçüm DAHA AĞIR bir şey buldu.** Ölçüm: `wren/mcp_server.py` **var** (707 satır, **18 araç**: `query_cube`·`list_cubes`·`describe_cube`·`get_context`·`recall_queries`·`run_sql`·`store_query`·`dry_run`…) ama bağımlılığı (`mcp` SDK) **kurulu değil** → `wren serve mcp` bu ortamda **koşamıyor**. ⊘ Hizalama reddediliyor, dört gerekçeyle: ① iki yüzey **farklı soyutlama düzeyinde** — onlarınki motor/katalog API'si, bizimki **cevap merdiveni** (`route`·`interpret`·`contribution.decompose`·`viz.recommend` karşılığı YOK) ② onların kümesinde **`run_sql` ve `store_query`** var; bizimki **salt-okuma** (`§C3②` yapısal olarak kanıtladı) — ortak sözlük tam da `§38.4`'ün yasakladığı yönde **yanlış bir beklenti** üretirdi ③ ad değişikliği `KAT-1`'i kırar (araç adı ↔ fiil beyanı `§D6` ile bağlı; eşleme tablosu = üçüncü kayıt) ④ kurulu olmayan bir yüzeye hizalanmak **hiçbir şeye** hizalanmaktır. 🔴🔴 **AMA ASIL BULGU:** `route`'un `required` alanları **`['question','schema']`** idi ve **hiçbir araç şema DÖNDÜRMÜYORDU** — yani sıfırdan başlayan bir ajan merdivenin **birinci basamağına hiç ulaşamıyordu**. Ölçüm **beş** aracı böyle buldu (`route`·`deterministic_refine`·`contribution.report`·`cross_cube_add` → `schema`; `yoy.compute` → `service`). ✅ Beşi de `report.compose`'un **zaten var olan** desenine taşındı (`enjekte=(...)`), `route` artık yalnız **`['question']`** istiyor. Ayrıca **`katalog`** keşif aracı eklendi (`katalog_metni.envanter`, tek sahip — `/stats/katalog` ile aynı fonksiyon; `yan_etki=yok`, yazma yüzeyi büyümedi). Kapı: `test_HICBIR_ARAC_CAGIRANDAN_SUNUCU_NESNESI_ISTEMIYOR`. ⊙ *Bir aracı yayımlamak, onu çağrılabilir yapmaz — çağıranın verebileceği şeyleri istemek yapar.*
| **`skills/`** | §11.3 | ✅ **B10** |
