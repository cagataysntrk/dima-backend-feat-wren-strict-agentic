...
| ✅ | `A12` | 🔴🔴 **KIYASLANABİLİRLİK ÖN KOŞULU** — `gercek_dunya.nufus_imzasi` + kapıda `⊘ KIYASLANAMAZ` dalı | ⊙ Bu kalem **planlanmamıştı**; `D8` turu onu **bedelini ödeterek** öğretti. Kataloğa üç boyut eklendi, kapı `doğru 95→79 · sessiz_yanlis 8→10` deyip **GERİLEME** etiketi bastı ve üç ayrı biçim denendi — üçü de kırmızı. Sonra in-process A/B koşuldu (aynı şema, tek fark üç boyut): **1072 gerçek korpus sorusunda DEĞİŞEN: 0.** Route davranışı **birebir aynıydı**. 🔴 Sebep: korpus sorularını **katalogdan türetiyor**; katalog büyüyünce üretilen küme baştan sona kayıyor ve toplam sayılar **iki farklı popülasyondan** geliyor. Kapı *«ürün geriledi»* ile *«katalog büyüdü»*yü **ayıramıyordu**. ⚠ Bu, `KAPI-DEFTERI`'nin ilk kaydının **ayna görüntüsü**: orada `gitas` düşünce payda 445→342 indi ve doğruluk **YÜKSELDİ** (sistem bozulurken sayı iyileşti); burada sistem **hiç değişmedi**, sayı **kötüleşti**. **Çözüm bir eşik değil bir ÖN KOŞUL:** taban soru kümesinin imzasını (`n:özet`) taşır; ayrışırsa kapı `⊘ KIYASLANAMAZ` der — deponun **zaten sahip olduğu** `⊘ ÖLÇÜLEMEDİ` kavramının kardeşi — ve **fail-closed**: yeşil değil, ama `GERİLEME` etiketi de **basmaz** (davranış hakkında bilgi yok). Soru-bazlı sapma listesi yine basılır. ⊙ Tam liste yerine **özet** seçildi: liste ~2286 anahtar (~140 KB) ve her katalog değişiminde git'te çalkalanırdı; gereken bilgi tek bir bit — *«aynı popülasyon mu?»*. Kapı `test_nufus_kiyaslanabilirligi.py` (9), ve *aynı nüfusta gerileme hâlâ yakalanıyor* satırı kapının **fazla ileri gitmediğini** kilitliyor. **Bedeli ölçüldü:** bu ayrımı yapamamak bugün **dört kapı koşumuna** (≈32 dk) mal oldu. *Bir metriğin kıpırdaması, ölçtüğü şeyin kıpırdadığı anlamına gelmez — paydanın bileşimi de bir değişkendir.* |
...
| ◐ | `B8` | **`T-3` kanonik varlık ekseni** — faz 1'in **kaydı düzeltildi**, faz 2 **ölçüldü ve ertelendi** | Faz 1 *«üç aile, kesişim YOK»* diye kaydetmişti; o kayıt `§DK` öncesi **yalan söyleyen enum'lardan** yazılmıştı ve **yanlıştı** — anahtar her iki ailenin tablosunda var. ⊙ **Grain endişesi de fazla büyütülmüştü:** `blend` bir **boyut** üzerinde birleşir, olgu satırlarında değil; canlı kanıt `{M1001 · ciro ₺40.480.479 · şikayet 928}`. 🔴 Faz 2'nin gerçek engeli grain **değil**, `_match_cube` tie-break'i: bkz. `D8`. *Bir borcu ölçmeden büyütmek onu ödenemez göstermenin en kolay yoludur* — ve ölçmeden **küçültmek** de aynı hatanın öteki yüzüdür |
...
| ☐ | # | iş | biten sayılır: |
|---|---|---|---|
| ✅ | `C1` | **Kapalı seçim** biçimi — ve 🔴 **biçim ÖLÇÜMLE bir adım daha ileri gitti: seçim bile SORULMUYOR, ÇIKARILIYOR** | ⊙ Kapalı seçim doğru bir tasarımdı ve kuruldu (`cube_router.eslesen_terim_oku`: model bir ad **seçer, uyduramaz** — dönen ad küpün beyaz listesinde sınanır; açık üretimde bu doğrulama **mümkün değildi**). 🔴 Ama canlıda **iki kez** ölçüldü ve model alanı **hiç doldurmadı**: (1) düzyazı talimat *«isteğe bağlı»* → alan yok · (2) `Biçim:` şablonunda **«ZORUNLUDUR»** → yine yok. Üç turda üçünde de **doğru çevirdi** (`zayiat→toplam_fire_kg` · `hasılat→toplam_ciro` · `alıcı→musteri`) ama çevirisini **söylemedi**. *Bir modelden cevabı taşımayan bir alanı doldurmasını istemek, ona bir dipnot yazdırmaktır; cevabı verir, dipnotu atlar.* ⊙ İstem eklentisi **geri alındı** (3941→3562 karakter; her Intent turunda `k=3` kez ödenen saf maliyet), okuyucu **silinmedi** — şema-yetenekli bir sağlayıcı alanı garanti edebilir ve o gün istem tek satırla döner |
...
| ✅ | `C3` | **`C3-D` — eşleme MODELDEN İSTENMEZ, ÇIKARILIR** (`cube_router.eslesen_terim_cikar`) | 🔴 **Kök çözüm, ve sinyaller ZATEN elde:** sistem her turda `bilinmeyen=zayiat` (route'un çözemediği kelime) ve `measures:[toplam_fire_kg]` (garsonun seçtiği ad) hesaplıyor **ve ikisini de ize yazıyor**. Eşleme bu ikisinin **kesişimidir** → **0 ek token · 0 sağlayıcı bağımlılığı · 0 prompt riski**. Kural bilerek dar: tam **bir** bilinmeyen · route'un kendi sözlüğüyle **ulaşabildiği** adlar elenir · geriye tam **bir** hedef kalmalı. ⊙ *«alıcı bazında ciro»* bu yüzden çalışır — `ciro` route'un sözlüğünde **var** (elenir), `musteri` **yok** (kalır). Ve *«alıcı bazında hasılat»* doğru **susar** (iki bilinmeyen). **Canlı:** `«zayiat»→toplam_fire_kg` · `«hasilat»→toplam_ciro` · `«alici»→musteri` · `«kayip»→toplam_fire_kg`. **Hasat bağlandı** (`sinonim_hasadi.eslesmeleri_oku`, kaynak `trace_json` — sütun **zaten vardı**, göç gerekmedi): 4 kanıtlı eşleme, `«zayiat»` eşiği geçti. `C5`'in üç sert kuralı burada da geçerli: çelişkili eşleme (`bakiye`) kuyruğa **girmez**. ⊙ Fark büyük: *«`zayiat` bilinmiyor»* bir **sorudur**; *«`zayiat`=`toplam_fire_kg`, 14 kez böyle çözüldü»* bir **cevaptır** — ilki bir insanın düşünmesini, ikincisi yalnız **onayını** ister. Kapı `test_ters_yon_alani.py` (18) |
...
| ✅ | `D2` | **`F2` `oylama_paydasi`** → **`beta`** | ⊙ Kapattığı şey bir kusur değil bir **YALAN** ve aritmetiği kanıtlandı (`test_oylama_paydasi.py`): 1 cevap + 2 çekimser → payda `cands` ile **%100** *«oy birliği»*, payda `oylar` ile **%33** → eşiğin altında → **netleştirme**. 🔴 *Şüphenin en yüksek olduğu durum, sistemin en emin göründüğü durumdu.* Çekimser bir **bilgidir**; onu paydadan düşürmek **hayır oylarını saymadan oy birliği ilan etmektir**. ⚠ **Korpus kanıtı beklenmedi ve sebebi yapısal:** `nl_corpus` `rule` ile koşar, oylama yolu orada **hiç çalışmaz** — korpus kanıtı beklemek *hiç gelmeyecek bir ölçümü beklemek* olurdu. Enstrüman `A2` kaseti: kayıtlı oylar aynı, yalnız **payda** değişir → **sıfır API'lik bir bayrak A/B'si**. `on` şartı yapısal alana yazıldı — *ve onu oraya bir kapı istedi* |
| ✅ | `D3` | **`F1` `varsayilan_donem`** → **`beta`** | ⊙ Korpus A/B: doğru-cube **%94,9 → %95,0**, 🔴 `sessiz_yanlis` **8 → 8 (DEĞİŞMEDİ)**, payda 2286 sabit → `F1.1`'in durdurma şartı **tetiklenmedi**. Beyan curl ile doğrulandı (`F1.3`) ve **birleşiyor**: *«⏱ Dönem belirtmedin — verinin son 12 ayı alındı (01.06.2025 – 30.06.2026). … «fire» birden fazla yerde tanımlı — bu cevap **parti** tanımıyla hesaplandı.»* İki ayrı belirsizlik, tek cümlede, ikisi de itiraf edilmiş. 🔴 **Ve ölçümün ne ÖLÇTÜĞÜ düzeltildi:** korpus kazancı ölçmüyor, **bedeli** ölçüyor — korpus soruları katalogdan üretiliyor ve **çoğu zaten dönem taşıyor**; %13,7'lik netleştirme yükü orada değil **gerçek kullanıcı turlarında**. Yani `+%0,1` bir kazanç ölçüsü değil bir **zarar yokluğu** ölçüsüdür, ve bayrağın açılma gerekçesi tam olarak budur. `on` şartı yapısal alana yazıldı. ⚠ Açık borç: `varsayilan_donem` yalnız **cevap üretilebilen** dalda çalışıyor; garson yanlış bir küp seçtiğinde yuva sorusu hâlâ ateşliyor — o bir dönem kusuru değil bir **sipariş** kusuru (`§DK` turunda ölçüldü) |
...
| ✅ | `D6` | **`F3` netleştirme ikilisi** — 🔴 **ÇIKIŞ ÖLÇÜTÜ KARŞILANDI, AMA BAŞKA BİR MEKANİZMAYLA** (`§6.4`: kalem çürüten ölçümle işaretlenir) | ⊙ Raporun çıkış ölçütü *«`bakiye` ₺11,86M vakası kapandı»* idi ve **canlıda ölçüldü**: `bu yıl bakiye` → **₺11.859.052,65** — *aynı sayı* — ama artık **sessiz değil**: *«bakiye birden fazla yerde tanımlı — bu cevap **cari hesap** tanımıyla hesaplandı. Diğerleri: bakiye (mizan).»* **+ tıklanabilir mizan chip'i**. 🔴 Kapatan şey `netlestirme_onceligi` **değil**, `§Cİ/T` sahiplik ifşası + `B6` sahiplik kararlarıydı — ikisi de rapor yazıldıktan **sonra** indi. Dört vakada birden doğrulandı (`bakiye` · `toplam fire` · `alacak` · `ilk seferde tamam`; sonuncusu gerçekten belirsiz olduğu için **netleştirmeye** düştü, doğru davranış). ⊙ Ve bu, `D6`'nın ikili çerçevesini (*tahmin et ↔ sor*) **çürütüyor**: üçüncü bir davranış ikisinden de iyi — **cevapla, seçimi beyan et, alternatifi chip yap**. Kapsam kaybı **sıfır**, sessiz-yanlış **sıfır**. `netlestirme_onceligi` `off` kalıyor ve gerekçesi artık bir tahmin değil bir **ölçüm**: kapatacağı 53 vakanın karşılığı zaten ifşayla veriliyor; onu açmak **kapsamı bedelsiz yere** daraltırdı. *Bir ihtiyacı gideren şey çıktığında, o ihtiyaç için tasarlanmış plan bir borç olmaktan çıkar* (`A6`'nın aynı dersi) |
| ✅ | `D7` | **`F8` `llm_sema_kisitli`** — 🔴 **KARAR YAZILDI: bayrak `beta` kalır, sağlayıcı DEĞİŞMEZ.** Üç ölçüm, üçü de bu turda | **`F8.1` token israfı — zaten kapalı:** `plan_garson.py:181` şemayı yalnız `sema_kullanir` doğruysa kuruyor → destekleyemeyen sağlayıcıda **hiç üretilmiyor**; raporun ~10.000 token'ı **yok** (`B5` kapatmış). **`F8.2` kapatacağı redler — sıfır:** canlı `/stats/plan` → `ulasilmaz 3 · bilinmeyen 1`, **ad reddi SIFIR**. Şema kısıtı **ad alanını** bağlar; `ulasilmaz` bir **graf** özelliğidir (bir adım hiç koşmayan bir çıktıya işaret ediyor) ve hiçbir JSON şeması onu engelleyemez. ⚠ Dürüstlük kaydı: `n=9`; ama sınıf argümanı `n`'e bağlı değil. **`F8.3` üçüncü kullanım — ve kendi ölçümüyle düştü:** turun ortasında şema kısıtının bir işe daha yarayacağını gördüm — bir alanın **üretilmesini garanti etmek** (`C3`'ün tam ihtiyacı) — ve `D7` gerekçemi düzeltmeye hazırlandım. Sonra `C3-D` eşlemeyi **deterministik olarak çıkardı** ve o ihtiyaç **ortadan kalktı**. ⊙ Ayrıca `§DK-2` ile **değer** ekseni kapandı ve şema kısıtı o ekseni **yapısal olarak kapatamaz** (değerler veriye bağlı, sınırsız). ⚠ `G6.11` garantinin yokluğunu **zaten makbuza yazıyor**. *Bir sağlayıcıyı değiştirmenin bedeli ölçülmüş bir modeli ölçülmemişle takas etmektir; gerekçesi de ölçülmeli.* |
| ◐ | `D8` | **`F4`/`T-3`** — 🔴🔴 **EKSEN KIRIK DEĞİL (ölçüldü), ama AÇMANIN BEDELİ DE ÖLÇÜLDÜ: `sessiz_yanlis +2` → GERİ ALINDI** | ⊙ **(1) Tarif yanlıştı:** `F4` *«en büyük iş»* diye planlanmıştı (kanonik varlık katmanı + `blend` dolayımı). `§DK` enum'ları düzeltince ölçüm bunu çürüttü — anahtar **her iki ailenin tablosunda duruyordu**: ticari 4 küp `musteri_kod` (`M1001…`), üretim ailesinin `partiler`/`tamir_rework` tablolarında `musteri_kod` **zaten var** (katalogda kapalı), `cari_kodu` ise bir **cari** kodu (17 değer, 3 tür — müşteri diye açmak yalan olurdu). ⊙ **(2) Yetenek KANITLANDI:** üç küpe boyut açılınca **7 küp tek anahtarda** oldu ve orkestratör *«müşteri kodu bazında ciro ve şikayet sayısı»*na **3 adımda** cevap verdi (SORGU+SORGU+MATRIS): `{M1001 · ciro ₺40.480.479 · şikayet 928}`. Raporun *«yapısal olarak kurulamıyor»* dediği soru **kuruldu**. 🔴 **(3) Ama bedeli kapı ölçtü — ÜÇ koşumda kararlı:** `doğru 95→79` · `devir 2142→2160` · **`sessiz_yanlış 8→10`**. Üç ayrı biçim denendi (geniş sinonim · sinonim+etiket yok · teknik-ad sinonimi) ve **son ikisi birebir aynı sayıyı** verdi → sorun sinonimde değil **boyutun varlığında**. ⟳ **MEKANİZMA İDDİAM ÇÜRÜTÜLDÜ — ve çürüten şey kendi ölçümüm oldu.** Önce `_match_cube`'un `dim_owners == 1` tie-break'ini suçladım ve bunu doğrulamadan yazdım. Sonra **in-process A/B** koştum (kapı maliyeti sıfır): aynı şema, tek fark üç boyut; **1072 gerçek korpus sorusunda DEĞİŞEN: 0**. 🎯 **Yani route davranışı kanıtlanabilir biçimde AYNI.** Gerileme ürünün cevaplarında değil, **korpusun ürettiği soru kümesinde**: üreteç soruları da beklentileri de katalogdan türetiyor; üç boyut eklenince **payda +2** oldu ve sınıf dağılımı kaydı. 🔴 Bu, deponun iki kez kaydettiği sınıfın **ayna görüntüsü** (`gitas` düştü, payda 445→342, doğruluk **YÜKSELDİ**): orada sistem bozulurken sayı iyileşmişti; burada **sistem değişmedi, sayı kötüleşti**. *Bir metriğin kıpırdaması, ölçtüğü şeyin kıpırdadığı anlamına gelmez — paydanın bileşimi de bir değişkendir.* ⚠ **Açık kalan tek soru, ve dar:** `payda +2` ile `sessiz_yanlis +2` **aynı sayıda** — yani iki yeni sessiz-yanlış, büyük olasılıkla yeni boyut için **üretilen iki yeni soru**. Doğrulanmadı (o koşumun raporu üzerine yazıldı) ve doğrulanmadan **kapanmayacak**. ⚠ **Karar:** geri alındı — `F1.1`/`E-2`: *kapsam kazancı sessiz-yanlışla satın alınmaz*. Geri alma sonrası kapı **yeşil** ve taban **birebir** (`doğru=95 · sessiz=8 · payda=2286`), yani gerilemenin tamamı bu değişiklikti (`C3-D` ve `§FÇ` korpus-nötr — ayrıca ölçüldü: `§FÇ` 1072 korpus sorusunun **sıfırında** yeni eşleşme üretiyor). 📋 **KALAN İŞ — gerekçesi DEĞİŞTİ, kendisi değil:** `birlesme_anahtari: true` beyanı hâlâ doğru iş, ama sebebi *«route kırılıyor»* **değil** (kırılmıyor); sebebi **bir iç anahtarın soru üretim yüzeyinde işi olmaması**. Kapsam: korpus üreteci **dışında**; `blend`, garson kataloğu ve `dimension_values` **içinde**. ⊙ Ve karar kuralı da netleşti: geri alma `sessiz_yanlis`'i gördüğü için doğruydu — ama o sayının **gerçek mi ölçüm artefaktı mı** olduğu bir sonraki turda ayrılacak; ayrılmadan yeniden açılmayacak. *Bir vetoyu anlamadan kaldırmak, onu hiç koymamakla aynıdır.* |
...
### F · KATALOG↔VERİ AYRIŞMASI *(`§DK` — bu raporda YOKTU, curl turu buldu)*

> 🔴 Bu bölüm planlanmamıştı. `D3`/`D6` doğrulanırken koşulan 24 senaryoluk curl turu,
> raporun hiçbir kaleminde geçmeyen bir kusur sınıfı çıkardı: **kataloğun kendisi
> garsona yalan söylüyordu.** Ve sınıfın **üç ayrı kopyası** vardı.
>
> ⊙ Raporun kendi kuralının (*«beklenmedik bir sayıda önce ÖLÇÜM YÜZEYİNDEN şüphelen»*)
> beşinci kez doğrulanışı — ama bu sefer şüphelenilecek yüzey **ürünün kendisiydi**.

| ☐ | # | iş | biten sayılır: |
|---|---|---|---|
| ✅ | `F1` | **`§DK` — enum, sorgunun kullanacağı İFADEDEN okunur** (`wren_service._enrich_cube_dim_values`) | Kısayol boyut **ADIYLA** eşleşiyor, arama tablosu **80 modelin kolonlarını tek sözlükte** düzleştiriyordu → ad çakışınca başka bir modelin ham kolonu küpün **kendi ifadesini gölgeliyor**, `expression` satırına **hiç gelinmiyordu**. **43 boyutun ifadesi adından farklı**, **5'i kanıtlanmış yalancı**: `parti.musteri`·`kalite.musteri` → `M1001…` (oysa *TOROS ÖRME TİC. LTD. ŞTİ.*) · `kalite.tedarikci` → `T-101…` · `kalite.vardiya`·`makine_duruslari.vardiya` → `1·2·3` (oysa *«1. Vardiya (08-16)»*). 🔴 Faturası bir kolaylık değil bir **sessiz yanlış**: *«bu yıl **1. vardiyada** fire oranı»* → **3 vardiya birden**, ilk satır *«3. Vardiya»*, süzgeç sessizce düştü, **beyan yok**. ⊙ Düzeltme kısayolu **kaldırmadı, kanıta bağladı** (`(base, expr)` anahtarı) — ve yalnız yalanı kesmekle kalmadı, **doğruyu bedavaya** verdi: `musteri_ad` zaten kendi base'inin örneklenmiş kolonu olduğu için motora **hiç gidilmedi**. Kapı `test_katalog_enum_ifadeden.py` (7) |
| ✅ | `F2` | **`§DK-3` — route ile garson AYNI menüye bakar** (`cube_router.boyut_degerleri` + `deger_eslesmeleri`) | 🔴 **Aynı kusurun ikinci kopyası, ve route'un içinde.** `§DK` kataloğu düzeltti ama route değerleri oradan **okumuyordu**: kendi eşleşmesini yine ham model kolonlarından **boyut adıyla** yapıyordu. Ölçüm: `vardiya` → route'un baktığı yerde **kolon HİÇ YOK** (süzgeç yapısal olarak imkânsız) · `musteri`/`tedarikci` → kullanıcının **asla yazmadığı** kodlar. Tek kaynağa bağlandı; üstüne **çekirdek eşleşmesi** (kullanıcı `«1. vardiya»` yazar, katalog `«1. Vardiya (08-16)»` tutar → sondaki parantez atılır, **tek aday kalırsa** eşleşir). ⚠ Sınır korundu: `«ram»` iki makineyi çağırdığı için **susar** — *gevşetilen bir kural, gevşetildiği yerde tahmin üretir*. Canlı: `%16,78` ve `0,635` (önce: süzgeçsiz üç satır). Kapı `test_deger_kaynagi_tek.py` (7) |
| ✅ | `F3` | **`§DK-2` — süzgeç değeri çapası** (`app/deger_capasi.py`, bayrak `deger_capasi`) | Garson süzgecin **değerini de kendisi yazar** ve varlığını hiçbir şey sormuyordu: `makine eq "Bakım"` → **0 satır, beyan yok**. Bir uydurma **sayı** değil bir uydurma **yokluk** — ve sıfır bir cevap gibi göründüğü için daha sinsi. ⚠ `route()` bu doğrulamayı `value_index` ile **zaten** yapıyordu ama **yalnız route yolunda**; kural huniye kondu → merdivenin **her** basamağında geçerli (*bir kural yalnız bir basamakta geçerliyse, o kural değil bir tesadüftür*). Üç sonuç, üçü de beyanlı. 🔴 **Yanlış-pozitif yapısal olarak dışarıda:** yalnız **tam** enum'larda konuşur — `wren_service` bir boyutun değerlerini ancak sayılabildiğinde yazar, çok değerli boyutta kayıt **hiç yoktur** → *«listede yok»* gerçekten yokluktur (`§101.1`). Kapı `test_deger_capasi.py` (12) |
| ✅ | `F4` | 🔴 **SIRA BİR TASARIM KARARIDIR** — ve bu satır onu kaydeder | `F3` (`§DK-2`) kapısı `F1` (`§DK`) **düzeltilmeden yazılamazdı**: o gün enum'lar yalan söylüyordu ve kapı `müşteri`/`vardiya` sorgularının **doğrularını** reddederdi — kusuru düzeltmek yerine **kilitlerdi**. *Bir doğrulayıcı, doğruladığı kaynaktan daha güvenilir olamaz.* ⊙ Ve kapının kendisi **aynı turda** bir borç doğurdu: `vardiya eq "3"` için *«listede yok»* dedi ve **haklıydı**, ama kullanıcının kastettiği besbelliydi → **tekil önek** yolu eklendi (`«3» → «3. Vardiya (00-08)»`, **55.512 dk**). *Dürüst bir red bir başarı değil, çözülecek bir borçtur* — kullanıcının kuralı, kapının kendi çıktısına uygulandı |
