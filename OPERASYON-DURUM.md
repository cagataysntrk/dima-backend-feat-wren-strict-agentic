...
| ✅ **G6.3·G6.4·G6.5** | **KAPANDI** — `referans: {eksen, kaynak, hedef}` (kapalı beş-eksen sözlüğü, **derleyicisi olan bir tanesi**), taşıyıcı `Niyet.referans` bir **türev** (alan değil → ikinci sahip yapısal olarak imkânsız), Intent-JSON'a **`blend`** girdi (`Ç-14` geri alındı, `$defs` ile `N²`→`N`). 🔴 Yan kazanç: `blend_uyumlu` iki çağıranın **tek** yüklemi oldu ve eski tek satırın **filtre körlüğünü** kapattı. Bayrak: `referans_dili` (`beta`). Kapı: `tests/test_referans_dili.py` (14) |
| 🔴 **G6 · ölçüm borcu** | Planın *"Intent-JSON `referans` üretebiliyor (**canlı**)"* maddesi **karşılanMADI** ve karşılanmış gibi gösterilmiyor: bu koşumda gerçek sağlayıcı yok. `G3.2` ile **aynı** alet sorunu — `eval --slice llm` (bugün **4 vaka**) büyümeden ölçülemez |
| 🔴 **VQR KAPATILDI — kurtarmaktan çok BOZUYORDU** *(kullanıcı kararı, 2026-08-07)* | `DIMA_VQR_ACIK=0` (varsayılan). ⊙ Gerekçe ölçüldü: doğrulanmış soru deposu merdivenin **İLK** basamağı, yani bir kayıt dondurulduğu an deterministik yolun sonraki iyileşmelerini **es geçtiriyor**. Canlı: *«şubatta ciro ocağa göre nasıl değişti»* → `source=vqr`, 434 ms, ve cevap **beyanlı kısmi** (`eksik_niyet=['kiyas','trend']`) — aynı soruyu `Ö10` ile düzelttiğimiz hâlde kullanıcı **eski cevabı** görecekti. 🔴 *Bir öğrenme deposu, öğrendiğini ne zaman unutacağını bilmiyorsa öğrenmez — ezberler; ve ezber, düzeltilen kusuru da korur.* ⚠ **Mekanizma SİLİNMEDİ, kapatıldı** (`MIMARI §10`): depo · benzerlik kapısı · güven kapısı · şema-sürüm kapısı · `/verify` ucu yerinde. Testleri de silinmedi — `vqr_acik` fixture'ıyla mekanizmayı açıkça açıyorlar (*kapatılan bir mekanizmanın testleri silinirse, açıldığı gün kimse onu sınamaz*). 🔴 **YENİDEN AÇMANIN ÖN KOŞULU CİDDİ AR-GE:** (a) bayatlama politikası — kayıt ne zaman geçersizleşir (şema sürümü yetmiyor: **router** iyileşince de geçersizleşmeli); (b) **eksik/kısmi cevap asla kaydedilmemeli** (bu turda kapatıldı ama depo zaten kirlenmiş olabilir); (c) doğrulama **curl ile canlı** yapılmalı — bu kusur birim testinde değil **canlı turda** göründü. |
| 🔴 **ANLATICI ZAMAN AŞIMI SINIRI YOK** *(canlı ölçüm)* | Aynı sağlayıcı aynı iş için **2.936 ms ↔ 22.564 ms** yaptı (**8 kat**), ve *flash* adlı bir modelde. `t2_anlatici` çağrısının **zaman aşımı/failover eşiği yok**: 22 saniyelik bir süsleme için kullanıcı bekliyor. ⚠ `t2_sablon` basit vakaları artık LLM'e hiç göndermiyor, ama **karmaşık vaka bu sınırla korunmuyor**. Eşik ölçülmeli. |
| ✅ **KATALOG SÖZLÜĞÜ — KAPANDI** *(canlı denetim → `§AJ2`)* | `app/katalog_metni.py` (yeni; `build_catalog` `cube_router`dan taşındı → tavan **1760→1739**). cube + ölçü sinonimleri `«…»` içinde, dedup edilmiş; bayrak `katalog_sozlugu: beta`. ⚠ **Ajanın sayısı düzeltildi**: *+%54* denmişti, ölçüm **2,47×** dedi → boyut sinonimleri kapsam dışı bırakıldı (**1,93×**). 🔴 Kalan iki borç: **ölçüm** (`eval --slice llm`, 4 vaka) ve **önbellek** (`B6` — bu modül ilk müşterisi). ⟵ eski kayıt: `build_catalog` LLM'e **yalnız teknik kolon adları** yazıyor (`- oee: measures[ort_oee, …]`); şemadaki `synonyms`/`display` katmanı (`oee → verim, randiman, performans`) **hiç girmiyor**. Oysa `route()` tam o katmanı kullanıyor. ⊙ Canlı ölçüm: 9/9 Intent çağrısı `{"cube":null}` — oylama uyuşmazlığı değil, **aday yokluğu**. 🔴 Mimari **tersine dönmüş**: *en çok güvendiğimiz basamak, en kör hâliyle koşuyor.* Maliyet ölçüldü: **+2.192 token (%54)** — ama **sabit önek**, yani `B6` (önbellekleme) ödendiğinde tekrar etmeyen maliyet |
| 🔴 **G3.1–G3.3** | **`G3`'ün kararı ASKIDA ve ölçülemedi.** Geri alma (`%95,1→%93,5`) `lab/nl_corpus.py` ile yapıldı — o araç **tanımı gereği `rule` sağlayıcıyla** koşuyor (*"LLM yok: deterministik çekirdeğin tavanı"*). Yani ölçülen şey, turu **mutfağın en aptal yedeğine** devretmenin maliyeti. Üretimde o basamakta gerçek bir LLM var. 🔴 `G3.2` bir **kod işi değil ölçüm aleti işi**: `eval --slice llm` (bugün **4 vaka**) büyütülmeli. *Bir devrin maliyetini, devredilenin en kötü hâliyle ölçmek, devri değil yedeği ölçer.* |
| ⊘ **G3.5** | `app/merdiven.py` **bilerek yazılmadı.** Mekanizma bir kez yazılmış, ölçülmüş ve geri alınmıştı (`test_kisa_devre_yok.py::test_DAVRANIS_DONUSUMU_OLCULDU_VE_ERTELENDI`). Bu turda yeniden yazıldı ve **geri alındı**: bağlanmamış modül = **yetim** (`OPERASYON.md §6/1`). *Bir mekanizmayı ölçemeden kurmak, kararı ertelemenin pahalı bir biçimidir.* |
| ⊘ **G3.6·G3.7** | `G3.5`'e bağlı — aday defteri olmadan ne kapı yeniden yazılır ne `explain.path` zenginleşir |
| ✅ **G3.4** | **KAPANDI** — *cevapsız kesme oranı* `lab/nl_corpus.py`'ye eklendi. Ölçüt yasağın kendi cümlesinden türetildi: **cevap yok ∧ kullanıcı durdurmadı ∧ Discovery hiç koşmadı**. Üçüncü şart ayırt edici: merdiven sonuna kadar koşup bulamadıysa bu **kapsam sınırı**, kesme değil. Ham sayılar da raporlanıyor (`kesme_sayi`/`kesme_payda`) |
| ✅ **tsc** | **KAPANDI** — `nightly.yml`'e `frontend-tsc` **ayrı işi** eklendi (`pnpm exec tsc --noEmit`). ⚠ Ayrı iş bilinçli: `kapi` işine gömseydik frontend kurulumu çöktüğünde ölçüm kapıları hiç koşmazdı. Kapı: `test_TSC_GECELIK_CIDA_KOSUYOR` |
...
| ⊘ **Ö2** | **BULGU BAYAT — borç ZATEN KAPALI.** Ölçüldü ve kapandı: **−%18,2 → −%4,5** (`kaldi` → `gecti`, korpus sabit), bayrak `olcu_ekleme_takibi` **`beta`** — kayıt bu dosyanın `:112` satırında. Denetim ajanı `cube_router.py`'nin docstring'ini okumuş (sorunu anlatıyor) ama kapanış kaydını görmemiş; docstring de çözümü anmıyordu → **düzeltildi**. 🔴 `K1`'in bu oturumdaki **üçüncü** örneği. ⟵ eski iddia: **sahipsiz:** `lab/sharding.py` — 44 konuşmalık sabit kohort, tur 1 **%63,6** → tur 5 **%45,5** = **−%18,2** (hedef −%10). Kümülatif `cube_query` taşıma **var** (`context.py:185` ham pencereyi 2 turla sınırlar, cq ayrı yankılanır) ama derinleşmede yine bozuluyor. *Bir mekanizmanın var olması, işini yaptığının kanıtı değildir* |
...
| ✅ **S6** | **KAPANDI** — `G1.7`: rozetler tıklanabilir ama **düzenlemez, düzenleyene GÖTÜRÜR** (kısayol: `scrollIntoView` + `focus` + vurgu). Planın istediği biçim olduğu gibi uygulanamazdı: `InterpretationBar` o alanların **dördünü zaten** düzenliyor (`measures`·`filters`·`timeDimensions`·`dimensions`) → ikinci sahip olurdu. ⚠ `cube` rozeti tıklanmaz: cube değişimi bir düzenleme değil **yeni bir sorudur**. Kapı: `test_ROZET_CAPALARI_DUZENLEME_CUBUGUYLA_ESLESIYOR`. ⟵ eski: `G1.7`: temellendirme rozetleri **tıklanabilir** olacaktı (`POST /cube`, 0 LLM); düz `<span>` kaldı |
...
| 🔴 **P3** | **Plan şema budamasından hiç söz etmiyor.** Ölçüldü (23 cube'luk demo): intent katalog metni **4.038 token**, Discovery şema prompt'u **16.767 token**, `oneOf` şeması **10.038 token** — ve o sonuncusu **üretilip ATILIYOR** (sağlayıcı `oneOf` desteklemiyor). Tasarım `belgeler/denetim/2026-08-07_DIKKAT-EDILECEKLER.md §4`'te dört ölçümle hazır |
...
| ✅ **belge** | **KAPANDI** — `G2` mimari kaydı yazıldı (üç tasarım kararı + kapılar + `KURAL_CAPA` tekrarının kaydı). ⚠ Geç yazıldığı ve `§13.6` ihlali olduğu **kaydın içinde** duruyor |
