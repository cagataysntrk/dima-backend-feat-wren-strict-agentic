...
| ◐ **Borç 1 YARISI KAPANDI** | Kimliğin **tek sahibi** kuruldu (`app/istek_kimligi.py`, ContextVar — bu depoda 3. kez); üç giriş noktası (HTTP · zamanlayıcı · MCP) kimliği **kendisi** kuruyor. 🔴 `motor_cls=on` hâlâ **39 kırmızı**, ama sebep DEĞİŞTİ: 36 çağrı sitesi değil, `WrenService`'i **doğrudan kuran test/lab yolları**. Kalan iş: fixture düzeyinde açık kimlik — **ürün kodunda sıfır değişiklik** |
