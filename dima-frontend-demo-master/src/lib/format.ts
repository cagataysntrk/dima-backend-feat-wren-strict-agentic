...
export function unitFor(col: string): Unit | null {
  ...
  // "_yuzde" son-eki (YoY %değişim: toplam_ciro_degisim_yuzde) → % — parasal kontrolden
  // ÖNCE gelmeli, yoksa içindeki "ciro/maliyet" ₺ eşler (YoY tablosunda "-24,6 ₺" hatası).
  if (/_yuzde$/.test(c)) return { suffix: "%" }; // zaten yüzde ölçeğinde (0-1 değil)
