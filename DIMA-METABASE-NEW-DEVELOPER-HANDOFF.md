...
## PERMANENT HANDOFF RULES

```text
METABASE COMPUTES ANALYTICS.
DIMA GOVERNS INVESTIGATION / EPISTEMICS / DECISION MEANING.

P17 InvestigationGraph != P19 causal/contribution structure.
P18 SATISFIED != causal relationship proven.
P18 policy direction != causal direction.
P16 claim state is not overwritten by P19.

LEADING CAUSE != CONTRIBUTION.
MULTIPLE MATERIAL CONTRIBUTORS ARE LEGAL.
INCONCLUSIVE IS A FIRST-CLASS SUCCESS.

NO LLM-GENERATED NUMERIC CONFIDENCE.
NO CORRELATION→CAUSE PROMOTION.
NO GENERIC CAUSAL GRAPH PLATFORM.
NO PYTHON ANALYTICS BY DEFAULT.
DO NOT TUNE COGNITION TO ONE PROBABILISTIC TEST TRAJECTORY.
```
