<!-- DIMA-V2-ACTIVE-OVERLAY -->
# 🔴 DIMA V2 — AKTİF GELİŞTİRME OTORİTE KATMANI

> **Dal:** `feat/ask-v2-mvp`  
> **Başlangıç:** `wren-bağımsız@869280db316d5bf3f76d3253b8b80e5609a000b9`  
> **Bu blok V2 geliştirmesi boyunca aşağıdaki eski belgenin üstünde okunur.**
>
> V2 için iki mühürlü aktif kaynak vardır:
>
> 1. `belgeler/plan/DIMA_NIHAI_DENETIM_VE_MIMARI_RAPORU.md`
>    — **hedef mimari / kök neden / normatif V2 gerekçe authority**.
> 2. `belgeler/plan/DIMA_NIHAI_UYGULAMA_YOL_HARITASI.md`
>    — **uygulama sırası / P fazları / exit gate authority**.
>
> Yaşayan tek ilerleme kaydı:
> `belgeler/plan/DIMA_V2_GELISTIRME_DURUM.md`.
>
> Bu `MIMARI.md` dosyasının aşağıdaki mevcut gövdesi SİLİNMEZ. V2 açısından rolü:
>
> - current/legacy çalışan davranışın kanonik fotoğrafı,
> - reuse edilen auth / tenant / Wren / security / contract değişmezlerinin kaynağı,
> - hedef raporda supersede edilmemiş mevcut altyapı gerçeklerinin authority'si.
>
> **Çelişki çözümü:** Bir konu V2'nin hedef semantic/conversation mimarisiyse nihai rapor
> kazanır; implementation sırası ve scope konusunda roadmap kazanır. Mevcut çalışan
> altyapı/security gerçeğinde, hedef rapor açıkça değiştirmedikçe bu MIMARI gövdesi
> kazanır. Legacy davranış V2 semantic authority olarak kopyalanmaz.
>
> **Her V2 implementation adımı:** aktif P bölümünü oku → onun atıf yaptığı R bölümünü
> oku → R3/R3A hata sınıfını kontrol et → durum defterine ticket kartı yaz → kodla.
>
> **Mühür kuralı:** rapor ve roadmap geliştirme sırasında düzenlenmez. Yeni gerçekler,
> borçlar, commit'ler ve ilerleme yalnız durum defterine yazılır.
>
> **Test/hız kuralı:** küçük adımlarda yalnız gerekli hedefli test; büyük kapı milestone /
> demet sonunda. V2'nin amacı eski evi temizlemek değil, yeni beynin çalışan dikey
> dilimlerini hızlıca kanıtlamaktır.
...
| 🔴 **Cevapsız bir dalla cevaplı bir yolu KESME** *(18. — `KAT-2`, yol haritası §G/AJ0)* | Bir dal `source=None` dönüyorsa **henüz cevap üretmemiştir**; kendisinden geniş bir yolu (Intent-JSON · planlayıcı · Discovery) kesmesi kullanıcıya **daha kötü bir cevabı garanti eder**. Merdiveni yalnız **pozitif cevap** ya da kullanıcının **açık `yol_siniri`**'si bitirebilir. ⚠ Kural Discovery'yi herkese yeniden AÇMAZ: *"yalnız deterministik"* diyen kullanıcı yine kesilir — **ama kullanıcı öyle dediği için, bir yazım tahmini öyle dediği için değil.** ⊙ Ölçüldü: `değişti→eğitim` · `bunu→gunu` · `bundan→unvan` · `enerji kaynağı→enerji tep`; ve kodun **kendi yorumları aynı hatayı üç kez kaydetmiş** — her seferinde çağrı yerinde yamalanmış, **kapının kendisinde hiç**. Kapı: `tests/test_kisa_devre_yok.py` (13 dal envanterlendi, her biri gerekçeli). ⊙ **ÖLÇÜLDÜ (2026-08-06):** yasağın *tam* uygulaması (her netleştirmeyi adaya çevirmek) korpusu **%95,1→%93,5**, eval precision'ı **−%1,8** düşürdü ve süitte 7 kırmızı verdi → **geri alındı**. Sebep yapısal: `_try_fresh_intent` yalnız tahmin değil, **daraltılmış ama doğru** netleştirmeler de üretiyor; onları adaya çevirmek deterministik yolun kazandığı turları Discovery'ye devretti. 🔴 Yasağın **dördüncü koşulu** buradan doğdu: *bir sonraki basamak gerçekten daha yetenekli olmalı.* İnen: yasağın kendisi + envanter kapısı + yol haritasının adıyla andığı vakanın (`değişti→eğitim`) aday'a çevrilmesi. |
...
| `belgeler/mimari/Wren_Hibrit_GenBI_SaaS_Strateji.md` | **Mimari niyet belgesi — hâlâ geçerli.** Katman 5'i kelimesi kelimesine *"Semantic Compiler: Intent Query → SQL; **join graph**, metric math, RLS inject"*, §2.5 *"MDL graph'ta measure'ın model'inden dimensions'a **join yolu seçilir**"*. Yani join işi yeni bir yön değil, **strateji ile kod arasındaki boşluğun kapatılmasıdır.** |
| `belgeler/devir/*` | **Tarihsel kayıt. Normatif DEĞİL.** Ne zaman ne yapıldığını anlatır; #4 (küp-LLM stratejisi) bu belgeyle çelişen §3.1/§6.C teşhisleri içerir — **bu belge kazanır.** |
...
| `belgeler/urun/Dima-0-100-Gorev-Takip.md` | **ÜRÜN şartnamesi. Mimari otorite DEĞİL** — §8.3. |
...
| **0014** | Kimlik/yetki: her kimlik özniteliği **token'dan** türer. K6: **her veri erişimi kanıtlanabilir iz bırakır** (KVKK). Audit satırı yazılmadan başarı raporlanmaz | `control_plane/authorize.py`, `control_plane/audit.py` |
...
| **0018** | **Sinonim mimarisi 3 katman**: pack YAML ⊕ arketip ⊕ **canlı öğrenilen overlay (additive, asla silmez)**. Evrensel kural: **LABEL ⊆ SYNONYM**. Adaylar otomatik canlıya çıkmaz | `app/sinonim_onerici.py`, `SynonymOverride` |
