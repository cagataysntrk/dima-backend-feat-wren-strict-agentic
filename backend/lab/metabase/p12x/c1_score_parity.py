#!/usr/bin/env python3
"""P12X-C1 stock-vs-fork native capability parity scorer with bounded divergence repeats."""
...
def rows_match(observed: list[list[Any]], expected: list[list[Any]], comparison: str, tol: float) -> bool:
    if comparison in {"scalar", "single_row", "ordered_rows"}:
        return len(observed) == len(expected) and all(
            eq_row(a, b, tol) for a, b in zip(observed, expected)
        )
...
def scope_ok(a: dict[str, Any]) -> bool:
    ...
    return (
        isinstance(r, dict)
        and r.get("database_name") == EXPECTED_DB
        and isinstance(q, dict)
        and q.get("database") == r.get("database_id")
    )
...
def evaluate_pair(stock_art: dict[str, Any], fork_art: dict[str, Any], oracle: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "stock": stock,
        "fork": fork,
        "permissions_equal": stock_perm == fork_perm,
...
def main() -> int:
    ...
    if (
        oracle_all.get("corpus_fingerprint") != EXPECTED_CORPUS
        or case_set.get("corpus_fingerprint") != EXPECTED_CORPUS
    ):
        raise SystemExit("frozen corpus fingerprint drift")

    oracle_by = {
        x["id"]: x for x in oracle_all.get("results") or [] if isinstance(x, dict)
    }
    ...
    for case_id in case_set["case_ids"]:
        ...
        pair = evaluate_pair(stock_art, fork_art, oracle)
        stock, fork = pair["stock"], pair["fork"]

        if stock["pass"]:
            stock_pass += 1
            if fork["pass"]:
                retained += 1
        if stock["pass"] and fork.get("silent_wrong"):
            new_silent_wrong += 1
        if pair["permission_regression"]:
            permission_regressions += 1
        if pair["dataset_scope_drift"]:
            scope_drift += 1
        if bool(stock["pass"]) != bool(fork["pass"]):
    ...
    if args.phase == "discover":
        report = {
            "schema_version": "p12x_c1_parity_discovery_v1",
            "corpus_fingerprint": EXPECTED_CORPUS,
            "status": "DIVERGENCE_DISCOVERED" if divergent_case_ids else "NO_DIVERGENCE",
    ...
    if divergent_case_ids:
        if args.repeat_root is None:
            raise SystemExit("material divergence exists but --repeat-root was not supplied")
        for case_id in divergent_case_ids:
            oracle = oracle_by[case_id]
            attempts = []
            for attempt in range(1, REPEAT_COUNT + 1):
                stock_path = args.repeat_root / "stock" / f"{case_id}-{attempt}.json"
                fork_path = args.repeat_root / "fork" / f"{case_id}-{attempt}.json"
                if not stock_path.exists() or not fork_path.exists():
                    raise SystemExit(
                        f"missing required divergence repeat evidence: {case_id} attempt {attempt}"
                    )
    ...
    report = {
        ...
        "closure": closure,
        "cases": rows,
        "divergence_repeats": repeat_evidence,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(
        json.dumps(report, ensure_ascii=False, indent=2, default=str) + "\n",
